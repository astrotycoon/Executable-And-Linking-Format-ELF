...
The section holds a symbol hash table. A dynamically linked object file must contain a symbol hash table. Currently, an object file may have only one hash table, but this restriction may be relaxed in the future. See [``Hash Table''](xxx) in the Chapter 5 for details.
