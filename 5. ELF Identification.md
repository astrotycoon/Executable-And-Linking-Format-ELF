...
如何做到这些呢？答案就是ELF文件在一开始的位置（`e_ident`数组）就提供了这些信息，在获取这些信息后，就知道如何解析文件剩余的所有内容了。`e_ident`是***unsigned char***类型的数组，

而对单字节数据的解析是不需要考虑大小端问题的，所以不管在什么平台上，对这些信息的解析方式都是一样的，并且它们是独立于文件剩余内容的。

另外，我们使用一些tool（例如readelf）去解析ELF文件内容时，ELF文件的信息可能是与我们当前的操作系统和处理器架构无关的（比如我们在x86（小端）linux操作系统上使用readelf去解析一个SPARC处

理器（大端）solaris操作系统上的ELF文件）。
