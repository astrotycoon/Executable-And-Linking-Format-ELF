...
**SHT_HASH**|	The section header index of the symbol table to which the hash table applies.<br>类型为SHT_HASH的section中包含的是一个符号哈希表（symbol hash table）。 该类型的section的`sh_link`代表的是符号表的节索引值（section header index）。	|0
