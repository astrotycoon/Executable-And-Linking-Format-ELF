...
This member holds the section header table index of the entry associated with the section name string table. If the file has no section name string table, this member holds the value **SHN_UNDEF**. See [\`\`Sections"](xxx) and [\`\`String Table"](xxx) for more information.
