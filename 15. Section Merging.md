...
在可重定位文件中，可以给类型为**SHT_PROGBITS**的section设置属性标志**SHF_MERGE**（**SHF_MERGE**的描述见[Figure 4-11: Section Attribute Flags](https://github.com/astrotycoon/Executable-And-Linking-Format-ELF/blob/main/12.%20Section%20Attribute%20Flags.md#figure-4-11-section-attribute-flags)）。**SHF_MERGE**表明链接器可以将它与其它同样设置了**SHF_MERGE**标志的section合并。通过合并这样的section，可以有效地减小最终生成ELF文件的大小（最终生成的ELF文件可以是可执行文件，也可以是共享库文件）。
